...
const ResumeCard = ({resume: {id, companyName, jobTitle, feedback, imagePath}}: {resume: Resume}) => {
    ...
    useEffect(() => {
        const loadResume = async () => {
            const bloob = await fs.read(imagePath)

            if(!bloob) return;

            let url = URL.createObjectURL(bloob)
            setResumeUrl(url)
        }
        ...
        <Link to={`/resume/${id}`} className="resume-card animate-in fade-in duration-1000">
            ...
            {resumeUrl && (<div className="gradient-border animate-in fade-in duration-1000">
                <div className="w-full h-full">
                    <img src={resumeUrl} alt="resume-img" className="w-full h-[350px] max-sm:h-[200px] object-cover object-top" />
                </div>
            </div>)}
        </Link>
    )
